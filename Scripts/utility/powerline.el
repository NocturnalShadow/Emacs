...
(defpowerline row         "Line: %l")
(defpowerline rows-info	  
	(format "Line: %s%s "
	"%l"
	(if (> (length buffer-line-count) 0)
		(concat "/" buffer-line-count)
		" ")))
(defpowerline column      "%3c")
(defpowerline space       "%12")
...
(setq-default mode-line-format
              (list "%e"
                    '(:eval (append
				      ...
				      (powerline-rows-info     	'left        powerline-color1  powerline-color2  )

				      (powerline-vc      	'center                        powerline-color2  ))
			      
			      (powerline-pull-right (list
				      (powerline-space          'right       powerline-color1  powerline-color2  )
